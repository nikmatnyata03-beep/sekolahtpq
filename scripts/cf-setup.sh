...
#
# Yang dilakukan:
#   1. Buat database D1 "simadji-db" (jika belum ada) -> tulis database_id ke wrangler.jsonc
#   2. Buat bucket R2 "simadji-uploads" (jika belum ada)
set -euo pipefail
cd "$(dirname "$0")/.."
...
echo "==> Buat database D1 (simadji-db)..."
OUT=$(bunx wrangler d1 create simadji-db 2>&1 || true)
...
echo "==> Buat bucket R2 (simadji-uploads)..."
bunx wrangler r2 bucket create simadji-uploads 2>&1 | head -3 || true
