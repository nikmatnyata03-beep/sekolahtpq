...
// Task 40: ClassAssignmentEditor & levelLabel dipindah ke komponen bersama
// src/components/admin/class-assignment-editor.tsx agar dipakai juga di form akun.

/**
 * Bagian "Kelas yang Diampu" (Task 36): checkbox dikelompokkan per jenjang.
 * Kelas yang sudah punya pengampu lain tetap bisa diambil alih — ditandai jelas
 * agar admin sadar penugasan berpindah.
 */
...
export function TeachersAdmin() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const [t, c] = await Promise.all([apiGet<Teacher[]>('/api/teachers'), apiGet<ClassRoom[]>('/api/classes')])
      setTeachers(t)
      setClasses(c)
    } catch (e) {
        ...
        toast({
          ...
          description:
            form.classIds.length > 0
              ? `${form.fullName} ditambahkan dan ditugaskan ke ${form.classIds.length} kelas.`
              : `${form.fullName} berhasil ditambahkan. Tugaskan kelasnya lewat Edit bila belum.`,
        })
      }
      setFormOpen(false)
      await load()
